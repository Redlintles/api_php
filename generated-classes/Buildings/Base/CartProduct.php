...
/**
 * Base class that represents a row from the 'cart_products' table.
 *
 *
 *
...
abstract class CartProduct implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Buildings\\Map\\CartProductTableMap';
    ...
    public function setQuantity($v)
    {
        ...
        if ($this->quantity !== $v) {
            $this->quantity = $v;
            $this->modifiedColumns[CartProductTableMap::COL_QUANTITY] = true;
        }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(CartProductTableMap::COL_ID_PRODUCT)) {
            $modifiedColumns[':p' . $index++]  = 'id_product';
        }
        if ($this->isColumnModified(CartProductTableMap::COL_QUANTITY)) {
            $modifiedColumns[':p' . $index++]  = 'quantity';
        }

        $sql = sprintf(
            'INSERT INTO cart_products (%s) VALUES (%s)',
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(CartProductTableMap::COL_ID_PRODUCT)) {
            $criteria->add(CartProductTableMap::COL_ID_PRODUCT, $this->id_product);
        }
        if ($this->isColumnModified(CartProductTableMap::COL_QUANTITY)) {
            $criteria->add(CartProductTableMap::COL_QUANTITY, $this->quantity);
        }
     ...
     * objects.
     *
     * @param object $copyObj An object of \Buildings\CartProduct (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Buildings\CartProduct Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
