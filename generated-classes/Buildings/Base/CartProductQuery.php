 ...
 *
 * @method     ChildCartProductQuery orderById($order = Criteria::ASC) Order by the id column
 ...
 *
 * @method     ChildCartProductQuery groupById() Group by the id column
 ...
 *
 * @method     ChildCartProduct|null findOneById(int $id) Return the first ChildCartProduct filtered by the id column
 ...
 *
 * @method     ChildCartProduct requireOneById(int $id) Return the first ChildCartProduct filtered by the id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 *
 * @method     ChildCartProduct[]|Collection findById(int|array<int> $id) Return ChildCartProduct objects filtered by the id column
 * @psalm-method Collection&\Traversable<ChildCartProduct> findById(int|array<int> $id) Return ChildCartProduct objects filtered by the id column
...
abstract class CartProductQuery extends ModelCriteria
{
    ...
    public function findPk($key, ?ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = CartProductTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key)))) {
    ...
    {
        $sql = 'SELECT id, id_cart, id_product, quantity FROM cart_products WHERE id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            CartProductTableMap::addInstanceToPool($obj, null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key);
        }
        $stmt->closeCursor();
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        $this->addUsingAlias(CartProductTableMap::COL_ID, $key, Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        $this->addUsingAlias(CartProductTableMap::COL_ID, $keys, Criteria::IN);
    ...
    public function filterById($id = null, ?string $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(CartProductTableMap::COL_ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(CartProductTableMap::COL_ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        $this->addUsingAlias(CartProductTableMap::COL_ID, $id, $comparison);
    ...
    {
        if ($cartProduct) {
            $this->addUsingAlias(CartProductTableMap::COL_ID, $cartProduct->getId(), Criteria::NOT_EQUAL);
        }
